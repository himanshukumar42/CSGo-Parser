...
func main() {
	...
	if err != nil {
		log.Fatalln("parsing header error ", err)
	}
	...
	p.RegisterEventHandler(func(e events.ItemPickup) {
		...
		x, y := mapMetadata.TranslateScale(e.Weapon.Entity.Position().X, e.Weapon.Entity.Position().Y)
	...
	padding := float64(grenadeDotSize) / 2.0 // Calculating padding amount to avoid shrinkage by the heatmap library
	...
	imgHeatmap := heatmap.Heatmap(image.Rect(0, 0, bounds.Dx(), bounds.Dy()), data, grenadeDotSize, grenadeOpacity, schemes.AlphaFire)
	draw.Draw(img, bounds, imgHeatmap, image.Point{}, draw.Over)

	// Write to stdout
	err = jpeg.Encode(os.Stdout, img, &jpeg.Options{Quality: grenadeJpegQuality})
	if err != nil {
